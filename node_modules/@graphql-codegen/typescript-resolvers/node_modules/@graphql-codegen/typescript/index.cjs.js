...
class TsVisitor extends visitorPluginCommon.BaseTypesVisitor {
    ...
    FieldDefinition(node, key, parent) {
        ...
        return comment + visitorPluginCommon.indent(`${this.config.immutableTypes ? 'readonly ' : ''}${node.name}${addOptionalSign ? '?' : ''}: ${typeString}${type === 'class' ? ';' : ','}`);
    }
    InputValueDefinition(node, key, parent) {
        ...
        const comment = visitorPluginCommon.transformComment(node.description, 1);
        const { type } = this.config.declarationKind;
        return comment + visitorPluginCommon.indent(`${this.config.immutableTypes ? 'readonly ' : ''}${node.name}${addOptionalSign ? '?' : ''}: ${node.type}${type === 'class' ? ';' : ','}`);
    }
